...
NativeModule::NativeModule(const std::wstring& sLocation, DWORD dwFlags)
...
{
	if (GetFlags() & LS_MODULE_THREADED)
	{
		// Note:
...
bool NativeModule::Init(HWND hMainWindow, const std::wstring& sAppPath)
{
	...
	if (_LoadDll())
	{
		...
		if (GetFlags() & LS_MODULE_THREADED)
		{
			...
			m_hThread = (HANDLE)_beginthreadex(&sa, 0, NativeModule::ThreadProc,
		...
		{
			SetLoadTime(GetTickCount() - dwStartTime);
		}
		else
		{
			SetLoadTime((DWORD)((iEndTime - iStartTime) * 1000 / iFrequency));
		}
	}
...
bool NativeModule::_LoadDll()
{
	...
	if (!m_hInstance)
	{
		...
		if ((m_hInstance = LoadLibraryW(GetLocation())) != nullptr)
		{
...
#if defined(_WIN64)
			if (GetModuleArchitecture(GetLocation()) == IMAGE_NT_OPTIONAL_HDR32_MAGIC)
			{
...
#else
			if (GetModuleArchitecture(GetLocation()) == IMAGE_NT_OPTIONAL_HDR64_MAGIC)
			{
...
#endif
			else if (PathFileExistsW(GetLocation()))
			{
		...
		if (!bReturn)
		{
			LPCWSTR pwzFileName = PathFindFileNameW(GetLocation());
...
UINT __stdcall NativeModule::ThreadProc(void* dllModPtr)
{
	NativeModule* dllMod = (NativeModule*)dllModPtr;

#if defined(MSVC_DEBUG)
	LPCTSTR pszFileName = PathFindFileName(dllMod->GetLocation());
...
void NativeModule::HandleThreadMessage(MSG &msg)
{
	switch (msg.message)
	{
	...
	case WM_DESTROY:
	{
		NativeModule *dll_mod = (NativeModule*)msg.lParam;
